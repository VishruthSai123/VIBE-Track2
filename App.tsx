...
const SidebarItem: React.FC<{ 
    ...
    >
        <div className={`transition-colors duration-200 ${active ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'}`}>{icon}</div>
...
const AppContent: React.FC = () => {
    ...
    const renderView = () => {
        ...
        if (!activeProject && !['project-settings', 'workspace-settings', 'profile', 'teams', 'spaces'].includes(currentView)) {
             return (
        ...
        switch (currentView) {
            ...
            case 'project-settings': return <Settings view="project" />;
            case 'workspace-settings': return <Settings view="workspace" />;
        ...
        <div className="flex h-screen bg-white overflow-hidden relative">
            ...
            `}>
                ...
                <div className="w-64 bg-slate-50 border-r border-slate-200 flex flex-col flex-shrink-0 h-full">
                            ...
                            >
                                <div className="flex items-center space-x-3 overflow-hidden">
                                    ...
                                    <div className="text-left min-w-0">
                                        <h3 className="text-sm font-bold text-slate-800 truncate">{activeProject?.name || 'Select Project'}</h3>
                                    </div>
                                </div>
                    ...
                    {/* Active Space Banner in Sidebar */}
                    {activeSpace && (
                        <div className="px-4 pt-4">
                            <div 
                                onClick={handleSpaceBannerClick}
                                className="bg-indigo-600 text-white p-3 rounded-lg shadow-md flex flex-col cursor-pointer hover:bg-indigo-700 transition-colors group"
                            >
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-[10px] uppercase font-bold opacity-75">Active Space</span>
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); setActiveSpace(null); }} 
                                ...
                                <div className="font-bold text-sm flex items-center gap-2 truncate">
                                    <Box className="w-4 h-4 flex-shrink-0" /> {activeSpace.name}
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar">
                        ...
                        <SidebarItem 
                            ...
                            active={currentView === 'workspace-settings'}
                            onClick={() => handleViewChange('workspace-settings')} 
                        />
                        ...
                        <div className="pt-4 mt-4 border-t border-slate-200">
                            <SidebarItem 
                                ...
                                active={currentView === 'project-settings'}
                                onClick={() => handleViewChange('project-settings')} 
                            />
                        </div>
                    </nav>
            ...
            <div className="flex-1 flex flex-col h-full min-w-0 relative">
                ...
                <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-20 relative shadow-sm">
                    ...
                    <div className="flex-1 max-w-xl flex items-center">
                        {/* Breadcrumb if Space Active */}
                        {activeSpace && (
                            <div className="flex items-center mr-4 text-sm text-slate-500 bg-slate-50 px-3 py-1 rounded-full border border-slate-200 shadow-sm">
                                <Box className="w-4 h-4 mr-2 text-indigo-500" />
                                <span className="font-semibold text-slate-700 max-w-[100px] truncate">{activeSpace.name}</span>
                                <button onClick={() => setActiveSpace(null)} className="ml-2 hover:text-red-500 transition-colors p-0.5 rounded-full hover:bg-slate-200">
                                    <X className="w-3 h-3" />
                                </button>
                            </div>
                        )}
