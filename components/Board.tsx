...
export const Board: React.FC = () => {
  const { issues, users, updateIssue, activeSprint, completeSprint, searchQuery, activeSpace } = useProject();
  ...
  // Filter issues by sprint, search, user, AND Space (via context.issues)
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-3">
            <div>
                <div className="flex items-center space-x-3">
                    ...
                    {activeSpace && <span className="px-2.5 py-0.5 rounded-full bg-purple-50 text-purple-700 text-xs font-bold uppercase tracking-wide border border-purple-100 flex-shrink-0">{activeSpace.name}</span>}
                        ...
                        <div className="p-3 px-4 font-semibold text-xs uppercase text-slate-500 tracking-wider flex justify-between items-center bg-white/40 rounded-t-xl backdrop-blur-sm border-b border-slate-200/50 sticky top-0">
                            <div className="flex items-center space-x-2">
                                <div className={`w-2 h-2 rounded-full shadow-sm ${
                                    ...
                                    column.id === Status.QA ? 'bg-yellow-500' :
