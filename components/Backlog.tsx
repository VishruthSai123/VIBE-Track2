...
import { ChevronRight, ChevronDown, Plus, MoreHorizontal, X, GripVertical, Calendar, Loader2, FolderPlus } from 'lucide-react';
...
export const Backlog: React.FC = () => {
  ...
  const [createIssueSprintId, setCreateIssueSprintId] = useState<string | null>(null);
  ...
  const handleCreateSprint = async (e: React.FormEvent) => {
    ...
    const success = await createSprint(
        newSprintName || `Sprint ${sprints.length + 1}`, 
        newSprintGoal, 
        newSprintStartDate, 
        newSprintEndDate, 
        newSprintCapacity
  ...
  const handleOpenCreateIssue = (sprintId: string | null) => {
  ...
  if (!activeProject) {
      return (
          <div className="h-full flex flex-col items-center justify-center text-center p-8 text-slate-400">
              ...
              <p>Please select a project to view the backlog</p>
          </div>
      );
  }
                    ...
                    {!isCollapsed && (
                        <div className="divide-y divide-slate-100 min-h-[50px]">
                            {sprintIssues.length === 0 ? (
                                <div 
                                    className="p-8 text-center text-slate-400 text-sm border-dashed border-2 border-slate-200 m-4 rounded bg-slate-50/50"
                                    onDragOver={handleDragOver}
                                    onDrop={(e) => handleDrop(e, sprint.id)}
                                >
                             ...
                             {canCreateTask && (
                                <div className="p-2">
                                    <button 
                                        onClick={() => handleOpenCreateIssue(sprint.id)}
                                        className="w-full text-left p-2 text-slate-500 hover:bg-slate-100 rounded flex items-center space-x-2 text-sm transition-colors font-medium"
                                    >
        ...
        >
             ...
             <div className="divide-y divide-slate-100 min-h-[100px]">
                 ...
                 {canCreateTask && (
                    <div className="p-2">
                        <button 
                            onClick={() => handleOpenCreateIssue(null)}
                            className="w-full text-left p-2 text-slate-500 hover:bg-slate-50 rounded flex items-center space-x-2 text-sm transition-colors font-medium"
                        >
                            <Plus className="w-4 h-4" />
                            <span>Create issue</span>
                        </button>
